...
- 2026-09-06 当前同步结果：本地 `uv run ioskb stats` 为 1,080,698 块 / 56,827 向量，`freshness --skip-upstreams --check` clean；39 项知识库单元测试全部通过，SQLite `quick_check` 为 `ok`。生产认证问答自测于 00:13（Asia/Shanghai）11/11 全部通过。
