...
> 本文件随工作实时更新。最后更新：2026-08-03（本地索引、网站接口和生产数据同步完成）
...
| 9 | 全链路建库实测 | ✅ 完成 | 141,733 文件 / 1,069,080 块；46,145 块向量化；大镜像 1,022,935 块全部进 FTS |
...
| 16 | 细粒度知识卡片 | ✅ 完成 | 95 文件 / 1,191 块 / 1,191 向量；全量结构、路径、编号和行号审计通过 |
| 17 | 网站版 Agent 审查与生产同步 | ✅ 完成 | API 检索阈值、权威等级、引用校验、断流状态、无证据拒答和额度保护已修复；Vectorize 44,954 条、D1 FTS 44,954 条按稳定 ID 同步 |
...
- 本地网页 `/api/status`：141,733 文件、1,069,080 块、46,145 已向量化，模型离线预热完成；
- SQLite `quick_check=ok`，orphan、files.nchunks、vector 标记不一致均为 0。
