...
- 网站仓库：`/Users/tommywu/tommywu-lab` 的远端 `main` 当前为 `2dee16f`；用户工作区仍在 `fix/chat-input-initial-height`，含未提交首页改动与未跟踪素材，绝不在其中直接修改。本轮在隔离副本建立并推送 `codex/fix-ios-evidence-routing-live` 的 `81ac3ce`，尚未合并到 `main`、未部署生产：Apple 专有 API（如 WidgetKit/App Intents/Vision）会强制走 iOS 检索，重排分不能单独越过关键词/API 覆盖门槛，跨平台比较需要资料覆盖被问到的另一平台，否则 `422 no_evidence` 并退款；“只从 iOS 侧”才允许单侧说明。网站本地 23 项 Retrieval、14 项 API、TypeScript 和完整构建均通过。合并部署后，先定向重测 10 条 P0 no-evidence，再重跑完整 manifest；不要把未发布分支当成线上已修复。
