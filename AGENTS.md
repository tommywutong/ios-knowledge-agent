...
- 2026-09-07 网站检索修复已合并并部署：网站仓库 `/Users/tommywu/tommywu-lab` 的远端 `main` 现为 `9ce55d7`（`81ac3ce` 的后续修复）。GitHub Actions `34125568913` 全部通过，Cloudflare Pages production 为 `https://280999a7.tommywu-lab.pages.dev`，自定义域名 `https://www.tommywutong.cn` 已切换到同一提交；两个地址首页/API 均返回 HTTP 200，公开 GET 显示 `configured: true`。本次新增 v1 兼容路径的命名 Apple API 证据闸门，防止 WidgetKit 等问题被泛资料放行。原网站工作区仍保留用户的 `fix/chat-input-initial-height` 未提交修改和未跟踪素材，未被触碰。
- 2026-09-07 部署后 P0 定向复测：`pem-000013`（ARKit）、`pem-000014`（Core ML）、`pem-000026`（Android Handler vs iOS RunLoop）正确返回 `422 no_evidence`；`pem-000010`（WidgetKit）出现无 `done` 的异常流，`pem-000027`（Kotlin vs GCD）仍错误进入 `knowledge`。报告 `data/evaluation-results/production-eval-key-cases-20260907.json` 不保存回答正文。后续补丁已作为 `3dd5617` 提交在隔离网站分支 `codex/fix-ios-stream-and-cross-platform`，并通过回归测试、TypeScript 和完整构建：流式响应统一只写入一个终结事件，Kotlin/GCD 之类的跨平台比较必须具备双方证据；尚未推送、合并或部署，生产残余问题仍未消除。

- 2026-09-07 已完成 `XiyouMobile3G-iOS/ios-source-learning` 的**本地分层融合**：工作区根仓库为 `215ba1a`；`objc4-source` 已从旧探索目录的 `objc4-951.1` 切换到该工作区钉定的 `objc4-951.7`（`ebfe77e64331`）。接入 2,031 个受控源码/地图文件、17,586 块：`objc4-source`（975 块）和 `ios-source-maps`（173 块）为向量+FTS；CF、Apple libdispatch、Swift Foundation/Swift libdispatch、GNUstep 及四个第三方库共 16,438 块为 FTS-only，精确符号检索优先源码实现。Apple 公开源码、开源/跨平台参照、GNUstep 非 Apple 参照、第三方库和地图均有独立类型、权威等级与回答提示约束；地图只在本地 `search` 中作导航，不进最终问答或 Cloudflare 导出。`summer2026` 已排除整套工作区并清理此前误作为普通笔记入库的 154 个文件。当前本地为 1,092,820 块 / 52,511 向量，`quick_check=ok`、目标来源 freshness clean、55 项单测通过。**未生成发布包，未修改 Vectorize/D1/Pages，生产仍是 2026-09-06 快照。** `check-updates.sh` 的远端探测发现 Swift Foundation 有更新、CF/corelibs 探测超时；本次索引据上述已核对本地 commit，不宣称所有上游均为最新。
- 2026-09-07 已新增本地生产评测器 `scripts/run_production_eval.py`：默认只预检已人工复核的 130 条 manifest、证据账本、判分契约和追问夹具；只有 `--live` 才从现有 Keychain/环境令牌发起请求，先确认 `configured/authenticated/unlimited`，报告只保留模式、引用编号、来源元数据和长度，不保存回答正文，输出目录 `data/evaluation-results/` 被 Git 忽略。当前旧生产快照的 P0 初测为 6 passed / 23 failed / 2 skipped；10 条 P0 no-evidence 定向复测仅 Metal 光线步进正确返回 422，5 条误进 `knowledge`、4 条误进 `general`。这是当前生产路由/证据阈值的回归基线，不可归因于尚未发布的 `ios-source-learning` 语料。`pem-000001` 命中同一 WWDC 2021/10133 的中文译文，评测器已加入经人工核实的中英等价锚点；本轮工具与所有单测共 55 项通过。不要将这批 GLM 候选直接接入生产；先修复网站检索路由后再对新版语料重跑。
- 2026-09-07 已将后续 6 批 GLM 离线候选归档到 `data/glm/`：语义审查/黄金集、r2/r3 红队复核、四个薄弱主题种子、种子证据边界与 130 条生产评测清单、130 条证据账本与 98 条判分契约，共 3,479 条 JSONL 记录。每批自带校验器已在本仓库复跑通过；它们保留来源行号、证据/平台边界和失败归因，但始终是待审候选，未写入原始资料、索引、Vectorize、D1 或 Pages。
...
- 新版知识数据已发布：Vectorize `ios-kb` 为 55,635 条稳定 `v1-*` 向量；新主 D1 `tommywu-ios-kb-primary-20260905`（`IOS_DB`）正式表 84,818 行，归档 D1 正式表 40,000 行，合计 124,818 行 FTS v2 证据。网站最新生产 Pages 部署见上方 `280999a7`；此前 `2ed9ad9f` 为上一版本部署。
...
- 本知识库仓库此前资料变更已由 `feat/knowledge-sync-20260905` 快进合并到 `main` 并推送至 `origin/main`（`dec7c8d`）；当前 `main` 另有尚未推送的本地源码融合与评测器提交。除非用户明确要求“推送”，不要推送。
- 网站仓库：`/Users/tommywu/tommywu-lab` 用户工作区仍在 `fix/chat-input-initial-height`，含未提交首页改动与未跟踪素材，绝不在其中直接修改。本轮在隔离副本合并并推送 `main` 的 `81ac3ce` 与 `9ce55d7`；`9ce55d7` 补上 v1 兼容路径的命名 Apple API 证据闸门。最新 Pages 为 `280999a7`，CI run `34125568913` 全部通过。生产复测已证明 ARKit/Core ML/Android Handler 跨平台题可正确返回 `422`，但 WidgetKit 异常流与 Kotlin/GCD 误进 knowledge 仍待修复；不要把局部通过写成 P0 全部通过。
...
## GLM 5.3 Collaboration

GLM 5.3 在本项目中是低成本、高吞吐的**离线数据与评测协作者**。它的产物默认是待程序和人工
复核的候选，不是可直接进入生产的知识证据或部署变更。

- 可以批量产出：中英文别名与检索查询候选、可回答/no-evidence/跨平台干扰/明确追问评测题、
  主题/平台/API 或符号/资料质量标签、重复块/目录页/坏转换/低 iOS 相关性等资料质量发现、
  知识卡片或面试题草稿，以及按“资料缺失/召回/重排/路由/提示词”分类的失败报告。
- 结构化产物应存放在 `data/glm/` 的独立子目录，附带 schema、来源路径/行号、批次说明和覆盖统计；
  不得自动混入原始语料、`knowledge_cards/`、索引或生产导出。
- 每条候选引用的原始 `path` 与行号都必须真实存在；进入评测或卡片流程前，须由当前检索结果重新
  对齐证据、校验 schema、去重，并由人工审阅生产相关变更。
- 严禁修改 Obsidian、`26暑期内容`、源码镜像等原始资料；严禁写入 `.env`、密钥、Cookie 或 token；
  严禁直接修改 D1、Vectorize、Pages、Cloudflare 绑定、部署配置或生产数据；严禁把未核验的模型
  结论、知识卡片草稿当作 Apple 或内部源码事实证据；严禁合并、推送、部署、删除文件。
- 首个任务固定为“离线检索评测集”：先交付小批量 JSONL 候选与覆盖统计，等待程序/人工校验后再扩量；
  不生成最终问答正文，不宣称线上或本地检索已经成功。
